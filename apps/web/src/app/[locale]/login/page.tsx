...
export default function LoginPage() {
  ...
  useEffect(() => {
    const emailLabel = document.querySelector('#loginEmailLabel') as HTMLLabelElement | null;
    const emailInput = document.querySelector('#loginEmail') as HTMLInputElement | null;
    const passwordLabel = document.querySelector('#loginPasswordLabel') as HTMLLabelElement | null;
    const passwordInput = document.querySelector('#loginPassword') as HTMLInputElement | null;
    const showPasswordCheck = document.querySelector('#showPasswordCheck') as HTMLInputElement | null;
    const showPasswordToggle = document.querySelector('#showPasswordToggle') as HTMLLabelElement | null;
    const mySVG = document.querySelector('.svgContainer') as HTMLElement | null;
    const twoFingers = document.querySelector('.twoFingers') as SVGGElement | null;
    const armL = document.querySelector('.armL') as SVGGElement | null;
    const armR = document.querySelector('.armR') as SVGGElement | null;
    const eyeL = document.querySelector('.eyeL') as SVGGElement | null;
    const eyeR = document.querySelector('.eyeR') as SVGGElement | null;
    const nose = document.querySelector('.nose') as SVGPathElement | null;
    const mouth = document.querySelector('.mouth') as SVGGElement | null;
    const mouthBG = document.querySelector('.mouthBG') as SVGPathElement | null;
    const mouthSmallBG = document.querySelector('.mouthSmallBG') as SVGPathElement | null;
    const mouthMediumBG = document.querySelector('.mouthMediumBG') as SVGPathElement | null;
    const mouthLargeBG = document.querySelector('.mouthLargeBG') as SVGPathElement | null;
    const mouthMaskPath = document.querySelector('#mouthMaskPath') as SVGPathElement | null;
    const mouthOutline = document.querySelector('.mouthOutline') as SVGPathElement | null;
    const tooth = document.querySelector('.tooth') as SVGPathElement | null;
    const tongue = document.querySelector('.tongue') as SVGGElement | null;
    const chin = document.querySelector('.chin') as SVGPathElement | null;
    const face = document.querySelector('.face') as SVGPathElement | null;
    const eyebrow = document.querySelector('.eyebrow') as SVGGElement | null;
    const outerEarL = document.querySelector('.earL .outerEar') as SVGGElement | null;
    const outerEarR = document.querySelector('.earR .outerEar') as SVGGElement | null;
    const earHairL = document.querySelector('.earL .earHair') as SVGGElement | null;
    const earHairR = document.querySelector('.earR .earHair') as SVGGElement | null;
    const hair = document.querySelector('.hair') as SVGPathElement | null;
    const bodyBG = document.querySelector('.bodyBGnormal') as SVGPathElement | null;
    const bodyBGchanged = document.querySelector('.bodyBGchanged') as SVGPathElement | null;
    ...
    const TweenMax = {
      to: (targets: any, duration: number, vars: any) => {
        const nextVars = { ...vars };
        if (!hasMorph) {
          delete (nextVars as any).morphSVG;
          delete (nextVars as any).shapeIndex;
        }
        return gsap.to(targets, { ...nextVars, duration });
      },
      set: (targets: any, vars: any) => gsap.set(targets, vars),
      killTweensOf: (targets: any) => gsap.killTweensOf(targets),
    };
    const Power2 = { easeInOut: 'power2.inOut' };
    const Quad = { easeOut: 'power2.out' };
    const Expo = { easeOut: 'expo.out' };

    let activeElement: string | null;
    let curEmailIndex: number | undefined;
    let screenCenter: number;
    let svgCoords: { x: number; y: number };
    let emailCoords: { x: number; y: number };
    let emailScrollMax: number;
    const chinMin = 0.5;
    let dFromC: number | undefined;
    let mouthStatus = 'small';
    let blinking: gsap.core.Tween | undefined;
    let eyeScale = 1;
    ...
    let faceY: number;
    let faceSkew: number;
    let eyebrowSkew: number;
    let outerEarX: number;
    let outerEarY: number;
    let hairX: number;
    let hairS: number;
    ...
    function getPosition(el: Element) {
      ...
      while (current) {
        if (current.tagName === 'BODY') {
          const xScroll = (current as HTMLElement).scrollLeft || document.documentElement.scrollLeft;
          const yScroll = (current as HTMLElement).scrollTop || document.documentElement.scrollTop;
          xPos += (current as HTMLElement).offsetLeft - xScroll + (current as HTMLElement).clientLeft;
          yPos += (current as HTMLElement).offsetTop - yScroll + (current as HTMLElement).clientTop;
        } else {
          xPos += (current as HTMLElement).offsetLeft - (current as HTMLElement).scrollLeft + (current as HTMLElement).clientLeft;
          yPos += (current as HTMLElement).offsetTop - (current as HTMLElement).scrollTop + (current as HTMLElement).clientTop;
        }
        current = (current as HTMLElement).offsetParent as Element | null;
      }
    ...
    function initLoginForm() {
      ...
      if (emailInputEl.value !== '') {
        emailInputEl.parentElement?.classList.add('focusWithText');
      }
  ...
  return (
    <div className="yeti-login">
      <form onSubmit={handleSubmit} className="yeti-form">
            ...
            <svg className="mySVG" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 200 200">
              ...
              <g className="earR">
                ...
                <g className="earHair">
                  ...
                  <path fill="#FFFFFF" stroke="#3A5E77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" d="M146.6,62.8 c4.9,4.6,8.4,9.4,10.6,14.2c-3.4-.1-6.8-.1-10.1,0.1c4,3.7,6.8,7.6,8.2,11.6c-2.1,0-4.2,0-6.3,0.2c2.6,4.1,3.8,8.3,3.7,12.5 c-1.2-.7-3.4-1.4-5.2-1.9" />
                </g>
              </g>
              ...
              <g className="arms" clipPath="url(#armMask)">
                <g className="armL" style={{ visibility: 'hidden' }}>
                  ...
                  <path fill="#DDF1FA" stroke="#3A5E77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" strokeMiterlimit="10" d="M134.4,53.5l19.3-5.2c2.7-.7,5.4.9,6.1,3.5v0c.7,2.7-.9,5.4-3.5,6.1l-10.3,2.8" />
                  <path fill="#DDF1FA" stroke="#3A5E77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" strokeMiterlimit="10" d="M150.9,59.4l26-7c2.7-.7,5.4.9,6.1,3.5v0c.7,2.7-.9, 5.4-3.5,6.1l-21.3,5.7" />
                  <g className="twoFingers">
                    <path fill="#DDF1FA" stroke="#3A5E77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" strokeMiterlimit="10" d="M158.3,67.8l23.1-6.2c2.7-.7,5.4.9,6.1,3.5v0c.7,2.7-.9,5.4-3.5,6.1l-23.1,6.2" />
                    <path fill="#A9DDF3" d="M180.1,65l2.2-0.6c1.1-.3,2.2.3,2.4,1.4v0c.3,1.1-.3,2.2-1.4,2.4l-2.2,0.6L180.1,65z" />
                    <path fill="#DDF1FA" stroke="#3A5E77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" strokeMiterlimit="10" d="M160.8,77.5l19.4-5.2c2.7-.7,5.4.9,6.1,3.5v0c.7,2.7-.9,5.4-3.5,6.1l-18.3,4.9" />
                    <path fill="#A9DDF3" d="M178.8,75.7l2.2-0.6c1.1-.3, 2.2.3,2.4,1.4v0c.3,1.1-.3,2.2-1.4,2.4l-2.2,0.6L178.8,75.7z" />
                  </g>
                  <path fill="#A9DDF3" d="M175.5,55.9l2.2-0.6c1.1-.3,2.2.3,2.4,1.4v0c.3,1.1-.3,2.2-1.4,2.4l-2.2,0.6L175.5,55.9z" />
                  <path fill="#A9DDF3" d="M152.1,50.4l2.2-0.6c1.1-.3, 2.2.3,2.4,1.4v0c.3,1.1-.3,2.2-1.4,2.4l-2.2,0.6L152.1,50.4z" />
        ...
        <p className="yeti-subtitle">{t('welcomeBack')}</p>

        {error && <div id="loginError" className="yeti-error">{error}</div>}
        ...
        <div className="inputGroup inputGroup2">
          ...
          <label id="showPasswordToggle" htmlFor="showPasswordCheck">
            <span className="toggleLabel">Show</span>
        ...
        <div className="inputGroup inputGroup3">
          <button id="loginSubmit" type="submit" disabled={isLoading}>
            {isLoading ? '...' : t('submit')}
          </button>
        </div>

        <p id="loginFooter" className="yeti-footer">
          {t('noAccount')}{' '}
          <Link href="/signup" id="signupLink">
            {t('signup')}
          </Link>
        </p>
      </form>
