  ...
  useEffect(() => {
    ...
    const TweenMax = {
      to: (targets: gsap.TweenTarget, duration: number, vars: gsap.TweenVars) => {
        const nextVars = { ...vars };
        if (!hasMorph) {
          delete (nextVars as { morphSVG?: unknown }).morphSVG;
          delete (nextVars as { shapeIndex?: unknown }).shapeIndex;
        }
        return gsap.to(targets, { ...nextVars, duration });
      },
      set: (targets: gsap.TweenTarget, vars: gsap.TweenVars) => gsap.set(targets, vars),
      killTweensOf: (targets: gsap.TweenTarget) => gsap.killTweensOf(targets),
    };
  ...
  return (
    <div className="yeti-login">
      <form onSubmit={handleSubmit} className="yeti-form">
        ...
        <h1 className="yeti-title">Welcome back!</h1>
        <p className="yeti-subtitle">We&apos;re so excited to see you again!</p>
