...
The repository includes GitHub Actions workflows for CI and backend deployment to Render:
...
The CI workflow runs on `push` and `pull_request` and includes:
...
The Render deployment workflow triggers automatically after a successful `CI` run:

- `develop` deploys the backend to staging
- `main` deploys the backend to production

Create these GitHub repository secrets before enabling it:
...
Each secret must contain the deploy hook URL generated by the corresponding Render service.
